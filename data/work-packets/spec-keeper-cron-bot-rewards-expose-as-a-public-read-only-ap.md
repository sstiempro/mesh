# Work packet — Keeper/cron bot rewards → expose as a public read-only API endpoint + rate-limit

- **sig:** `spec:Keeper/cron bot rewards → expose as a public read-only API endpoint + rate-limit`
