# Work packet — Copy-trade vault (perf fee) → expose as a public read-only API endpoint + rate-limit

- **sig:** `spec:Copy-trade vault (perf fee) → expose as a public read-only API endpoint + rate-limit`
