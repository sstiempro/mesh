# Work packet — Copy-trade vault (perf fee) → turn the output into a daily auto-posted thread (attention → audience)

- **sig:** `spec:Copy-trade vault (perf fee) → turn the output into a daily auto-posted thread (attention → audience)`
- **source avenue:** edge:copy-trade-vault
- **drafted:** 2026-06-30T09:21:26.006Z
