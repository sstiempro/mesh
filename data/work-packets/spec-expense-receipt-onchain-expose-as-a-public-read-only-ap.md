# Work packet — Expense/receipt → onchain → expose as a public read-only API endpoint + rate-limit

- **sig:** `spec:Expense/receipt → onchain → expose as a public read-only API endpoint + rate-limit`
- **source avenue:** system:sp75
- **drafted:** 2026-06-30T13:45:38.657Z
- **routing (needs):** build
